...
from PyQt6.QtCore import (
    QByteArray, QBuffer, QDateTime, QEvent, QIODevice, QStringListModel, QPoint, QRect, QTimer, QUrl, Qt, QSize, pyqtSignal
)
...
from PyQt6.QtWidgets import (
    QApplication, QComboBox, QCheckBox, QCompleter, QDialog, QDialogButtonBox, 
    QFileDialog, QFontComboBox, QHBoxLayout, QLabel, QLineEdit, QListWidget, 
    QListWidgetItem, QInputDialog, QMainWindow, QMessageBox, QPushButton,
    QScrollArea, QSizePolicy, QSplitter, QSplitterHandle, QTabWidget, QTextEdit, 
    QToolBar, QVBoxLayout, QWidget, QWidgetAction, QMenu, QAbstractItemView  
)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.repaint()
        self.update()      
        self.update_checklist_completer()
        self.checklist_completer.popup().setVisible(True)

        """------------------------- Set Cursor Focus -----------------------""" 
        self.editor.setFocus()

        saved_size = app_settings.get("window_size", DEFAULT_WINDOW_SIZE)
        saved_position = app_settings.get("window_position", DEFAULT_WINDOW_POSITION)
        self.resize(saved_size[0], saved_size[1])
        self.move(saved_position[0], saved_position[1])

        # Apply saved panel visibility
        panel_collapsed = app_settings.get("panel_collapsed", False)
        if panel_collapsed:
            # If collapsed, set the sizes to collapse the panel
            self.splitter.setSizes([0, 1])
        else:
            # If not collapsed, set a default size for the panel
    ...
    def init_checklist_toolbar(self):
        """
        Initialize the Checklist toolbar with a search bar for launching checklists.

        - Creates a toolbar for managing checklists.
        - Adds a search bar with autocomplete functionality for checklists.
        """
        ...
        # Create a search bar for entering checklist names
        self.checklist_search_bar = QLineEdit(self)
        self.checklist_search_bar.setPlaceholderText("Search for a checklist...")  # Placeholder text
        self.checklist_search_bar.returnPressed.connect(self.search_and_launch_checklist)  # Connect to search handler

        # Prevent the search bar from auto-focusing at startup
        self.checklist_search_bar.setFocusPolicy(Qt.FocusPolicy.ClickFocus)

        checklist_toolbar.addWidget(self.checklist_search_bar)

        # Set up autocomplete for the search bar
        self.checklist_completer = QCompleter(self)
        self.checklist_completer.setCaseSensitivity(Qt.CaseSensitivity.CaseInsensitive)
        self.checklist_search_bar.setCompleter(self.checklist_completer)
        ...
        # Load available checklists into the completer
        self.update_checklist_completer()
        
        print("Completer attached:", self.checklist_search_bar.completer() is not None)
    ...
    def closeEvent(self, event):
        ...
        - Saves window size, position, and panel visibility to application settings.
        """
        ...
        # Save application settings (e.g., window size, position, and panel visibility) before exit
        app_settings["window_size"] = [self.size().width(), self.size().height()]
        app_settings["window_position"] = [self.pos().x(), self.pos().y()]

        # Update panel collapsed state based on its current visibility
        panel_collapsed = not self.toggle_panel_action.isChecked()  # Inverted logic for collapsed state
        app_settings["panel_collapsed"] = panel_collapsed
    ...
    def close_tab(self, index):
        ...
        # If this was the last tab, create a new one immediately
    ...
    def search_and_launch_checklist(self):
        ...
        if not search_term:
            QMessageBox.warning(self, "Invalid Input", "Please enter a checklist name.")
            return
    ...
    def open_checklist(self, checklist_name_or_path):
        ...
        try:
            ...
            if os.path.isfile(checklist_name_or_path):
                ...
                checklist_path = os.path.join(CHECKLISTS_FOLDER, f"{checklist_name_or_path}.json")
    ...
    def update_checklist_completer(self):
        """
        Update the checklist completer with the list of available checklists.

        - Scans the checklist directory for available JSON files.
        - Updates the autocomplete model for the search bar.
        """
        try:
            if not os.path.exists(CHECKLISTS_FOLDER):
                os.makedirs(CHECKLISTS_FOLDER)
                logger.info(f"Created missing checklist folder: {CHECKLISTS_FOLDER}")

            checklist_files = [
                os.path.splitext(f)[0]
                for f in os.listdir(CHECKLISTS_FOLDER)
                if f.endswith(".json")
            ]
            logger.debug(f"Checklist files found: {checklist_files}")

            if not checklist_files:
                logger.warning("No checklists found in the folder.")
                checklist_files = []

            checklist_model = QStringListModel(checklist_files, self)
            self.checklist_completer.setModel(checklist_model)
            logger.info(f"Autocomplete updated with {len(checklist_files)} checklists.")
        except Exception as e:
            logger.error(f"Failed to update checklist completer: {e}")
...
class SavedFilesPanel(QWidget):
    ...
    def collapse_panel(self):
        ...
        # Check current state from settings
        currently_collapsed = app_settings.get("panel_collapsed", False)
        if currently_collapsed:
            # Uncollapse the panel by setting a default size
            splitter.setSizes([200, 1])  # Adjust as needed for your default panel width
            app_settings["panel_collapsed"] = False
        else:
            # Collapse the panel
            splitter.setSizes([0, 1])
            app_settings["panel_collapsed"] = True
...
class ChecklistManager(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        if parent:
            main_window_position = parent.pos()
            self.move(main_window_position.x() + 50, main_window_position.y() + 50)  # Offset
        # Layout setup
        self.layout = QVBoxLayout(self)
...
class UnifiedChecklistEditor(QDialog):
    ...
    def populate_editor_area(self):
        ...
        for section in self.data.get("checklists", []):
            # Add the MainTitle
            if "MainTitle" in section:
                self.add_item_to_display({"type": "MainTitle", "text": section["MainTitle"]})

            # Add each item in the section
            for item in section.get("items", []):
                for item_type, text in item.items():
    ...
    def add_item_to_display(self, item):
        ...
        raw_text = item.get("text", "").strip()  # Always use raw text
        ...
        if item["type"] == "MainTitle":
            ...
            font.setPointSize(16)
            font.setBold(True)
            display_text = f"  {raw_text}"  # Indent
        elif item["type"] == "Description":
            font.setPointSize(12)
            display_text = f"    {raw_text}"  # Indent
        elif item["type"] == "Task":
            font.setPointSize(14)
            display_text = f"    • {raw_text}"  # Bullet point
        elif item["type"] == "ChecklistItem":
            font.setPointSize(12)
            display_text = f"        ☐ {raw_text}"  # Checkbox
        elif item["type"] == "ChecklistNote":
            font.setPointSize(12)
            font.setItalic(True)
            display_text = f"Note: {raw_text}"

        formatted_item = QListWidgetItem(display_text)
    ...
    def remove_selected(self):
        ...
        selected_item = self.editor_area.item(selected_index)
        item_text = selected_item.text().strip()

        # Determine whether the selected item is a MainTitle or part of a section
        for section in self.data["checklists"]:
            if section["MainTitle"] == item_text:
                # Remove the entire section
                self.data["checklists"].remove(section)
                break
            for item in section.get("items", []):
                item_type, text = next(iter(item.items()))
                if text.strip() == item_text:
                    # Remove the specific item
                    section["items"].remove(item)
                    break

        # Remove the item from the editor area
    ...
    def save_checklist(self):
        ...
        for i in range(self.editor_area.count()):
            ...
            # Retrieve raw text and type
            item_text = item_widget.text().strip()
            item_type = None

            # Infer the type based on font size and formatting
            font = item_widget.font()
            if font.pointSize() == 18 and font.bold():
                item_type = "MainTitle"
            elif font.pointSize() == 16 and font.bold():
                item_type = "SubTitle"
                item_text = item_text.lstrip()  # Remove display indentation
            elif font.pointSize() == 12 and not font.bold():
                if item_text.startswith("☐"):
                    item_type = "ChecklistItem"
                    item_text = item_text.replace("☐", "").strip()  # Remove checkbox
                elif item_text.startswith("Note:"):
                    item_type = "ChecklistNote"
                    item_text = item_text.replace("Note:", "").strip()  # Remove "Note:"
                else:
                    item_type = "Description"
                    item_text = item_text.lstrip()  # Remove display indentation
            elif font.pointSize() == 14:
                item_type = "Task"
                item_text = item_text.replace("•", "").strip()  # Remove bullet

            if not item_type:
                logger.warning(f"Unable to infer type for item: {item_text}")
                continue

            if item_type == "MainTitle":
                # Create a new section for MainTitle
                current_section = {"MainTitle": item_text, "items": []}
                structured_data["checklists"].append(current_section)
            else:
                # Add other types to the current section
                if current_section is None:
                    current_section = {"MainTitle": "Untitled Section", "items": []}
                    structured_data["checklists"].append(current_section)

                current_section["items"].append({item_type: item_text})
